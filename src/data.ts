/*
 * 【備忘録】overviewの書き方ガイドライン
 *
 * 採用担当者が重視するポイント：
 *
 * 1. アーキテクチャの採用
 *    - なぜその技術・構成を選んだのかを明記する
 *    - 他の選択肢と比較した上での意思決定の根拠を書く
 *
 * 2. 課題と対応
 *    - 「こういう課題があったので、こういう対応をした」という構成で書く
 *    - 問題の背景 → 自分が取った行動 → 結果、の流れを意識する
 *    - 「なぜそうしたのか」を深掘りされることを前提に、根拠を書いておく
 *
 * 3. 担当領域
 *    - 自分が具体的に何を担当したかを明確にする
 *    - 設計・実装・レビュー・意思決定など、関与の深さがわかるように書く
 *
 * 4. 意思決定への関与
 *    - 技術選定・設計方針・優先順位付けなど、意思決定に携わった経験を盛り込む
 *
 * 5. チームビルディング
 *    - チーム構成・役割分担・コミュニケーション上の工夫などを書く
 *    - メンバーの育成・採用・文化づくりへの貢献があれば記載する
 *
 * ※ レジュメ更新したら採用担当者に連絡すること
 */
