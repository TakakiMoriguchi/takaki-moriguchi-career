export const data = [
  ...
  },
  {
    ...
    project_detail: {
      ...
      tools_technologies_used: ['LucidChart', 'Sketch', 'AWS', 'M4u'],
  ...
  },
  {
    ...
    project_detail: {
      overview: `
        前回の案件から契約関係で、当案件にスライドして参画。¥n
        旧システムからのリプレイスで、Next.js(SSR)でフロントを作成する。¥n
        全体的なコンポーネント基盤や設定を作成。
      `,
      task_responsibilities: '開発',
      achievements: '10月末で契約終了',
  ...
  },
  {
    ...
    project_detail: {
      overview: `
        鉄道会社向けのWEBページ作成案件。¥n
        要件定義段階¥n
        打ち合わせ内容などを仕様書に記述、前任者が抜けるタイミングで、チェックインやふりかえりミーティングを引き継ぐ。¥n
        ライブラリなどの選定を行い、設計を提案。¥n
        ¥n
        フロントエンド実装¥n
        コンポーネントをどのレベルで分割し、どのように責務を分けるかを考慮し、１つも重複することなく設計を提案出来た。¥n
        FigJamを使用して納期管理や問題点などを可視化し、遅れがちな開発も期限内に完了。¥n
        バッチ修正¥n
        初めてのGo言語であったが、キャッチアップしつつ実装した。¥n
        ¥n
        案件通して、ふりかえりミーティングを積極的に行い、チームの結束力に貢献した。¥n
        スケジュールやタスク管理も担当し、期限内に完了できた。
      `,
      task_responsibilities: '開発',
      achievements: '開発終了',
  ...
  },
  {
    ...
    project_detail: {
      overview: `
        鉄道会社のWEBページ作成案件。¥n
        炎上案件に参画¥n
        既存コードが複雑であったため、コードのリファクタなどを行い、品質向上に貢献。¥n
        チェックインやふりかえりミーティングを積極的に行い、先ずは心理的安全性の確保を重視。¥n
        先ずはチームが安定して機能することを確保した。¥n
        その後はイシュー対応などを行い、バグの改善。¥n
        github-copilot を駆使し、リバースエンジニアリングで既存コードの解析を行い、要件定義や設計の更新に活かした。¥n
      `,
      task_responsibilities: '開発',
      achievements: '開発終了',
